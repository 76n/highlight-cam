package com.highlightcam.app.ui.theme

import androidx.compose.ui.graphics.Brush
import androidx.compose.ui.graphics.Color

object HC {
    val bg = Color(0xFF0A0A0A)
    val surface = Color(0xFF141414)
    val surfaceRaised = Color(0xFF1E1E1E)

    val white = Color(0xFFFFFFFF)
    val white60 = Color(0x99FFFFFF)
    val white40 = Color(0x66FFFFFF)
    val white20 = Color(0x33FFFFFF)
    val white10 = Color(0x1AFFFFFF)

    val green = Color(0xFF00F57A)
    val greenDim = Color(0x3300F57A)
    val blue = Color(0xFF3DBBFF)
    val blueDim = Color(0x333DBBFF)
    val amber = Color(0xFFFFAA00)
    val amberDim = Color(0x33FFAA00)
    val red = Color(0xFFFF3B3B)
    val redDim = Color(0x33FF3B3B)
    val onGreen = Color(0xFF001A0D)

    val gradientGreen = Brush.linearGradient(listOf(Color(0xFF00F57A), Color(0xFF00C45F)))
    val gradientDanger = Brush.linearGradient(listOf(Color(0xFFFF3B3B), Color(0xFFCC2A2A)))
}
